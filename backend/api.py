...
DIAGNOSTICOS: Dict[str, Dict[str, tuple]] = {
    'MQ7': {
        'ALTO': (
            ...
            'Causa: Temperatura de pirólisis del reactor demasiado baja. '
            'Acción correctiva: Incrementar la potencia del calentador en +15% para estabilizar el reactor.'
        ),
        'BAJO': (
            'CO bajo (Pobreza de gas)',
            'Causa: Falta de generación de gas por detención de reacción. '
            'Acción correctiva: Verificar que el alimentador de plásticos no esté obstruido.'
        )
    },
    'MQ135': {
        'ALTO': (
            'VOCs / Aromáticos elevados',
            'Causa: Craqueo térmico incompleto por flujo rápido. '
            'Acción correctiva: Reducir velocidad de inyección de gases para aumentar tiempo de residencia.'
        ),
        'BAJO': (
            'VOCs bajos (Bajo rendimiento)',
            'Causa: Reacción de pirólisis lenta. '
            'Acción correctiva: Aumentar la tasa de calentamiento térmico primario.'
        )
    },
    'MQ4': {
        'ALTO': (
            'Metano (CH4) elevado',
            'Causa: El reactor está por debajo del rango de craqueo óptimo para gasolina. '
            'Acción correctiva: Incrementar calefacción hasta estabilizar a 430°C.'
        ),
        'BAJO': (
            'Metano (CH4) bajo',
            'Causa: Temperatura excesiva o falta de craqueo primario. '
            'Acción correctiva: Monitorear termocupla central y ajustar límites térmicos.'
        )
    },
    'MQ2': {
        'ALTO': (
            'Gases livianos altos (GLP/Propano)',
            'Causa: Licuefacción ineficiente en el condensador. '
            'Acción correctiva: Aumentar el flujo de agua de refrigeración en el condensador secundario.'
        ),
        'BAJO': (
            'Gases livianos bajos',
            'Causa: Generación de vapores deficiente en el reactor. '
            'Acción correctiva: Aumentar la potencia del reactor principal.'
        )
    },
    'MQ3': {
        'ALTO': (
            'Contaminación por Oxigenados (Alcoholes)',
            'Causa: Contaminación de materia prima con plásticos oxigenados (PET/PVC). '
            'Acción correctiva: Detener alimentación y verificar clasificación previa de plásticos.'
        ),
        'BAJO': (
            'Trazas de oxigenados normales',
            'Causa: Comportamiento normal. '
            'Acción correctiva: No se requieren acciones correctivas.'
        )
    },
    'MQ9': {
        'ALTO': (
            'Combustibles medios elevados',
            'Causa: Desequilibrio de condensación o sobrecalentamiento local. '
            'Acción correctiva: Estabilizar la temperatura de la camisa del reactor.'
        ),
        'BAJO': (
            'Combustibles medios bajos',
            'Causa: Tasa de destilación muy lenta. '
            'Acción correctiva: Incrementar la rampa de temperatura del destilador.'
        )
    },
    'temp': {
        'ALTO': (
            'Temperatura de cámara elevada',
            'Causa: Calor radiante excesivo del reactor o falla del cooler. '
            'Acción correctiva: Activar extractor de la cámara y revisar aislamiento térmico.'
        ),
        'BAJO': (
            'Temperatura de cámara baja',
            'Causa: Temperatura ambiental fría o baja actividad de reacción. '
            'Acción correctiva: Estabilizar la temperatura de la cámara precalentándola.'
        )
    },
    'humedad': {
        'ALTO': (
            'Humedad de cámara elevada',
            'Causa: Fuga de vapor o trampa de agua saturada. '
            'Acción correctiva: Vaciar y purgar la trampa de agua/condensado antes del sensado.'
        ),
        'BAJO': (
            'Humedad de cámara baja',
            'Causa: Ambiente de sensado excesivamente seco. '
            'Acción correctiva: No se requieren acciones correctivas.'
        )
    }
}

# Dynamic import of TFLite with graceful fallback engine
tflite = None
try:
    ...
    try:
        ...
        try:
            ...
            tflite = None
            logger.warning(
                "No TFLite package found (tflite_runtime, ai_edge_litert, tensorflow.lite). "
                "Running in Z-Score statistical fallback mode.")


class CustomStandardScaler:
    """Lightweight pure-NumPy StandardScaler replacement that requires zero sklearn dependency."""

    def __init__(self, mean=None, scale=None):
        self.mean_ = np.array(
            mean if mean is not None else [
                23234.696, 22221.06066667, 22647.16666667, 2826.35933333,
                19540.74266667, 2792.15066667, 28.0286, 37.355
            ], dtype=np.float32)
        self.scale_ = np.array(
            scale if scale is not None else [
                767.81755271, 702.35999197, 680.48037363, 46.65735611,
                907.07035253, 49.13655767, 3.04302515, 8.30403807
            ], dtype=np.float32)
...
class CustomLabelEncoder:
    """Lightweight pure-Python LabelEncoder replacement that requires zero sklearn dependency."""

    def __init__(self, classes=None):
        self.classes_ = np.array(classes if classes is not None else ['ALTA', 'BAJA', 'MEDIA'])
    ...
    try:
        ...
        if tflite is not None:
            try:
                ...
                logger.warning("Failed to initialize TFLite interpreter: %s. Using statistical Z-Score engine.", err)
                assets.interpreter = None
        else:
            assets.interpreter = None
            logger.info("TFLite library not present. Z-Score statistical inference engine active.")
...
def read_root() -> HTMLResponse:
    """Serves the interactive quality control and pyrolysis diagnostics dashboard with anti-cache headers."""
    base_dir = os.path.dirname(os.path.abspath(__file__))
...
def _fallback_probabilities(ventana_normalizada: np.ndarray) -> np.ndarray:
    """Computes statistical probabilities based on Z-Score distance to reference profile."""
    if assets.perfil_referencia is not None:
        ref_vec = np.mean(assets.perfil_referencia,
                          axis=0) if assets.perfil_referencia.ndim == 2 else assets.perfil_referencia
        diff_norm = float(np.linalg.norm(np.mean(ventana_normalizada, axis=0) - ref_vec))
    else:
        diff_norm = float(np.linalg.norm(np.mean(ventana_normalizada, axis=0)))

    if diff_norm < 1.5:
        return np.array([0.94, 0.04, 0.02], dtype=np.float32)  # ALTA
    elif diff_norm < 3.0:
        return np.array([0.08, 0.87, 0.05], dtype=np.float32)  # MEDIA
    else:
        return np.array([0.02, 0.06, 0.92], dtype=np.float32)  # BAJA
        ...
        if abs(diff) > 1.0:
            ...
            if sensor in DIAGNOSTICOS:
                ...
                if status_dev in rules:
                    ...
                    alert_title = f"Desviación {status_dev} en {sensor}"
                    recommendation = f"El sensor {sensor} está anormalmente {status_dev.lower()}."
                diagnostics.append({
                    "sensor": sensor,
                    "status": status_dev,
...
def process_prediction(raw_window: np.ndarray) -> Dict[str, Any]:
    """Applies scaler, runs inference using TFLite or Z-score fallback, and computes diagnostics.
    ...
    """
    if not assets.scaler or not assets.label_encoder:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Model assets (scaler/label_encoder) are not fully loaded or initialized."
        )
    ...
    # 2. Format for input tensor
    entrada = ventana_normalizada[np.newaxis, :, :].astype(np.float32)

    # 3. Invoke model prediction or statistical fallback
    if assets.interpreter is not None and assets.input_details is not None and assets.output_details is not None:
        try:
            ...
            logger.error("TFLite inference failed: %s. Using statistical fallback.", err)
            probabilidades = _fallback_probabilities(ventana_normalizada)
    else:
        probabilidades = _fallback_probabilities(ventana_normalizada)
    ...
    if not diagnostics and clase_predictiva == "ALTA":
        diagnostics.append({
            ...
            "recommendation": "El perfil sensorial es óptimo para la producción de biocombustible "
                              "de alta calidad. Continuar con los parámetros de operación actuales."
        })
        ...
        try:
            ...
            if getattr(app.state, 'is_recording', False):
                try:
                    ...
                    logger.info("¡Muestra N° %d registrada en CSV (Modo Manual Active)!",
                                app.state.recorded_samples_count)
            ...
            rec_act = getattr(app.state, 'is_recording', False)
            cnt_act = app.state.recorded_samples_count
            logger.info("Auto-analysis complete. Quality: %s | Ambient: %s | Recording: %s (%d)",
                        prediction_res["prediction"], ambient_status, rec_act, cnt_act)
        except Exception as err:
...
def get_reference_profile() -> Dict[str, Any]:
    """Returns pattern baseline (means, stds and voltages) of real commercial gasoline calibration."""
    ...
    for i, s in enumerate(SENSORES):
        v_calc = round(float(means[i] * (4.096 / 32768.0)), 3) if i < 6 else round(float(means[i]), 1)
        pattern[s] = {
    ...
    return {
        ...
        "quality_thresholds": {
            "ALTA": {"min_confidence": 90.0, "max_zscore": 1.5,
                     "badge": "Clase A - Dentro de Norma ASTM D4814"},
            "MEDIA": {"min_confidence": 70.0, "max_zscore": 3.0,
                      "badge": "Clase B - Desviación Tolerable / Advertencia"},
            "BAJA": {"min_confidence": 0.0, "max_zscore": 99.0,
                     "badge": "Clase C - Fuera de Especificación / Adulterada"}
        },
        "pattern": pattern
    }
