    ...
    for i, sensor in enumerate(SENSORES):
        ...
        if abs(diff) > 1.0:
            ...
            if sensor in DIAGNOSTICOS:
                ...
                alertas.append(
                    f"  [ALERTA] {sensor} {direccion} (desviación: {diff:+.2f} std dev)\n"
                    f"     Detalle: {titulo} -> {causa}"
                )
            else:
...
def main():
    ...
    if usar_simulacion:
        ...
        print("[SIM] Metadatos del lote cargados automáticamente de la simulación:")
    else:
    ...
    try:
        ...
        print("=" * 80)
        header_str = (
            "| Lote N° | Fecha y Hora        | Plástico | Temp. Máx | Frec | "
            "ADC | Latencia | RAM Usada | Pred (IA) | Confianza | Ground Truth | Concordancia |"
        )
        print(header_str)
        row_str = (
            f"| {lote_n:03d}     | {fecha_hora} | {tipo_plastico:8s} | {temp_max:9.1f} | "
            f"1.0  | 16  | {latencia_media:8.2f} | {ram_media:9.2f} | "
            f"{clase_predictiva:9s} | {confianza:9.2f}% | {etiqueta_real:12s} | {concordancia:12s} |"
        )
        print(row_str)
        print("=" * 80)
