...
def inicializar_csv_si_falta(filepath: str):
    if not os.path.exists(filepath) or os.path.getsize(filepath) == 0:
        with open(filepath, mode='w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["muestra_id", "origen_grifo", "timestep", "MQ2",
                            "MQ4", "MQ135", "MQ3", "MQ7", "MQ9", "temp", "humedad"])
...
async def recibir_telemetria(data: SensorDataPayload):
    ...
    if len(estado.buffer_actual) >= 30:
        ...
        try:
            ...
            print(
                f"\n❌ [ERROR DE PERMISO] No se pudo escribir en {target_csv}. "
                "¡Por favor cierre el archivo en Excel o Block de Notas!"
            )
...
def realizar_chequeo_salud():
    ...
    if fallas == 0:
        ...
        print("  ⚠️ RESULTADO: Se observaron algunas anomalías. Verifique el estado de las conexiones.")
    print("======================================================================")
...
def ejecutar_toma_manual(grifo_codigo: str, meta: int):
    ...
    actuales = estado.muestras_grifo_1 if grifo_codigo == "Grifo_1" else (
        estado.muestras_grifo_2 if grifo_codigo == "Grifo_2" else estado.muestras_grifo_3)
    ...
    while actuales < meta:
        ...
        actuales = estado.muestras_grifo_1 if grifo_codigo == "Grifo_1" else (
            estado.muestras_grifo_2 if grifo_codigo == "Grifo_2" else estado.muestras_grifo_3)
        msg_ok = (
            f"\n  ✅ ¡MUESTRA_{estado.total_muestras:03d} ({grifo_codigo}) GUARDADA EN CSV! "
            f"Progreso {grifo_codigo}: {actuales}/{meta}"
        )
        print(msg_ok)
